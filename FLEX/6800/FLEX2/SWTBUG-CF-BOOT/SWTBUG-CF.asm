...
TABLE FCB 'G' GOTO
 ...
 FCB 'D' DISK BOOT
; FDB DISK
 FDB CFBOOT	;LOAD THE FIRST SECTOR OF THE CF CARD
 FCB 'B' BREAKPOINT
 FDB BREAK
 FCB 'O' OPTIONAL PORT
 FDB OPTL
...
;
; COMPACT FLASH CARD FIRST STAGE BOOT LOADER
;
;
; CRAIG R.S. JONES
; VERSION 01  2020-09-24
;
;
CF_BASE    EQU $8070
CF_DATA    EQU 0 ;DATA REGISTER
CF_ERROR   EQU 1 ;(READ) ERROR REGISTER
CF_FEATURE EQU 1 ;(WRITE) FEATURE REGISTER
CF_SECCNT  EQU 2 ;SECTOR COUNT REGISTER
CF_LBA0    EQU 3 ;LBA0 (LS)
CF_LBA1    EQU 4 ;LBA1
CF_LBA2    EQU 5 ;LBA2
CF_LBA3    EQU 6 ;LBA3 (MS)
CF_COMAND  EQU 7 ;(WRITE) COMMAND REGISTER
CF_STATUS  EQU CF_BASE+7 ;(READ) STATUS REGISTER
;
; STATUS REGISTER BITS
;
CFCBUSY    EQU  $80      ;CARD BUSY
CFCRDY     EQU  $40      ;CARD READY TO ACCEPT COMMANDS
CFCDWF     EQU  $20      ;WRITE FAULT
CFCDSC     EQU  $10      ;CARD READY TO ACCEPT NEW COMMAND
CFCDRQ     EQU  $08      ;DATA REQUEST FROM CARD
CFCCORR    EQU  $04      ;DATA ERROR CORRECTED
CFCERR     EQU  $01      ;PREVIOUS COMMAND HAS AN ERROR
;
; CF CARD COMMANDS / SETTINGS
;
CMDREAD    EQU  $20      ;READ SECTOR
CMDFEAT    EQU  $EF      ;FEATURE SET COMMAND
ENABLE8    EQU  $01      ;ENABLE 8 BIT DATA TRANSFER FEATURE
LBAMODE    EQU  $E0      ;ENABLE LBA MODE
;
;DISK    EQU  $E28F       ;SWTBUG  - D COMMAND MINIFLOPPY DISK BOOT LOADER
;PROM    EQU  $C000       ;SWTBUG  - Z COMMAND JUMP TO PROM ADDRESS 
LDADDR  EQU  $2400       ;SECTOR LOAD ADDRESS
;CONTRL  EQU  $E152       ;RETURN TO SWTBUG
;
; CAN IT FIT INTO SWTBUG?
; THE D (DISK) FUNCTION IN SWTBUG USES BYTES $E28F TO $E2C1 = 32 BYTES! 

;        ORG  DISK
;	     ORG  PROM
        ORG  $E400          ;END OF SWTBUG
CFBOOT  BSR  CHKBSY
        LDX  #CF_BASE		
        LDAA #LBAMODE       ;B7=1, B6=1 LBA MODE, B5=1, B4=0 DRIVE 0, HS3-HS0=0
        STAA CF_LBA3,X	 
        LDAB #ENABLE8       ;ENABLE 8-BIT TRANSFERS
        STAB CF_FEATURE,X 
        STAB CF_SECCNT,X    ;NUMBER OF SECTORS TO READ
        LDAA #CMDFEAT     
        STAA CF_COMAND,X    ;SET FEATURE COMMAND 
        BSR  CHKBSY        
;
;  READ LBA 0 ( TRACK 0 SECTOR 1 ) READS A FULL 512 BYTE SECTOR
;  	 
        CLRB   
        STAB CF_LBA0,X
        STAB CF_LBA1,X
...
CBOOT1  BSR  CHKBSY
        ...
        BITA #CFCDRQ	
        BEQ  CBOOT2
        LDAA CF_BASE        ;READ DATA BYTE
        STAA 0,X
        INX
        BRA  CBOOT1	
CBOOT2  JMP  LDADDR	    ;CROSS YOUR FINGERS AND GO!
;CBOOT2  JMP  CONTRL         ;JUMP BACK TO SWTBUG
;
; IS CARD BUSY?
;	
CHKBSY  LDAA CF_STATUS
        BITA #CFCBUSY      
        BNE  CHKBSY
        RTS
        END
